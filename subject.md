...
### 7.1. Equipos C y D – Arquitectura Lambda
...
- Cargar en HDFS:
  ...
  - `delays_history_sample.csv`,
  - `weather_history.csv`.
- Procesar con PySpark y/o Spark SQL:
  - Limpieza y validación de datos.
  - Cálculo de métricas históricas de retraso por aeropuerto, aerolínea, ruta, franja temporal y condiciones meteorológicas.
- Cargar los resultados agregados en MySQL/SQL Server (tablas de hechos y dimensiones para Power BI).

**Speed layer**:

- Consumir en streaming desde Kafka:
  - `flights_rt_api` (vuelos actuales derivados de OpenSky).
  - `weather_rt_api` (meteorología actual derivada de OpenWeather).
- Implementar un job de Flink/Beam que:
  - Realice joins por aeropuerto y ventana temporal.
  - Calcule KPIs en tiempo casi real:
    - retraso estimado medio por aeropuerto,
    - número de vuelos con riesgo de retraso significativo asociado a mal tiempo,
    - niveles de riesgo (bajo/medio/alto).
  - Escriba resultados en Cassandra/MongoDB y, opcionalmente, en un topic `kpi_rt`.

**Serving layer**:

- Exponer:
  - Tablas históricas en MySQL/SQL Server.
  - KPIs en tiempo real en Cassandra/MongoDB.
- Conectar Power BI a estas fuentes para construir dashboards combinando histórico y tiempo real.
